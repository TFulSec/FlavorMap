...
if (fs.existsSync(MOCK_DB_PATH)) {
  ...
  mockUsers = [
    { Id: 'mock-admin-id', Email: 'admin@example.com', PasswordHash: bcrypt.hashSync('password123', 10), FullName: 'Admin User', Role: 'Admin', IsBanned: false }
  ];
...
const isPublicRestaurant = (restaurant) => restaurant.PublicationStatus === 'Published'
  && restaurant.VerificationStatus === 'Verified';

mockRestaurants.forEach((restaurant) => {
  restaurant.PublicationStatus = restaurant.PublicationStatus || 'Published';
  restaurant.VerificationStatus = restaurant.VerificationStatus || 'Verified';
  restaurant.VerifiedAtUtc = restaurant.VerifiedAtUtc || new Date();
  restaurant.LastVerifiedAtUtc = restaurant.LastVerifiedAtUtc || new Date();
  restaurant.SourceType = restaurant.SourceType || 'MockSeed';
});
  ...
  if (!pool) {
    try {
      if (process.env.FORCE_MOCK_DB === 'true') {
        const forcedMockError = new Error('Forced mock mode');
        forcedMockError.isForcedMock = true;
        throw forcedMockError;
      }
        ...
        request: () => {
          const req = {
            ...
            query: async function(queryStr) {
              ...
              if (queryStr && queryStr.includes('SELECT * FROM') && queryStr.includes('Users WHERE Email')) {
              ...
              if(queryStr && queryStr.includes('SELECT rv.Id, rv.Rating, rv.Comment')) {
                  ...
                  const revs = mockReviews.filter(x => x.RestaurantId === rest.Id && !x.IsHidden).map(rv => {
              ...
              if(queryStr && (queryStr.includes('SELECT') && queryStr.includes('FROM Restaurants'))) {
                  if (queryStr.includes('WHERE IsFeatured = 1') || queryStr.includes('IsFeatured = 1')) {
                      let featured = mockRestaurants.filter(r => isPublicRestaurant(r) && r.IsFeatured === true);
                  ...
                  if (queryStr.includes('WHERE IsNew = 1') || queryStr.includes('IsNew = 1')) {
                      let newest = mockRestaurants.filter(r => isPublicRestaurant(r) && r.IsNew === true);
                      if (this.parameters.City) {
                  ...
                  if (queryStr.includes('WHERE r.Slug = @Slug') || queryStr.includes('WHERE Slug = @Slug')) {
                      ...
                      const rest = mockRestaurants.find(r => r.Slug === slug && isPublicRestaurant(r));
                      if (rest) {
                  ...
                  // Filter
                  let filtered = mockRestaurants.filter(isPublicRestaurant);
              ...
              if(queryStr && queryStr.includes('SELECT r.Id, r.Name, r.Slug') && queryStr.includes('Bookmarks')) { // for getBookmarks
                  ...
                  const result = myBms.map(b => {
                      const r = mockRestaurants.find(x => x.Id === b.RestaurantId && isPublicRestaurant(x));
              ...
              if(queryStr && queryStr.includes('UPDATE Restaurants SET Name=@Name')) {
                  ...
                  if (rest) {
                      ...
                      rest.OwnerUserId = this.parameters.OwnerUserId || null;
                  }
                  return { recordset: [] };
              }
              if(queryStr && queryStr.includes('INSERT INTO Restaurants')) {
                  const newRest = {
                      ...
                      OwnerUserId: this.parameters.OwnerUserId || null,
                      CreatedAt: new Date()
                  };
              ...
              if (queryStr && queryStr.includes('INSERT INTO') && queryStr.includes('Users (FullName')) {
                 const newUser = {
                     Id: 'mock-' + Date.now(),
                     FullName: this.parameters.FullName,
                     Email: this.parameters.Email,
                     PasswordHash: this.parameters.PasswordHash,
                     Role: this.parameters.Role || 'User',
                     CreatedAt: new Date()
                 };
              ...
              if (queryStr && queryStr.includes('INSERT INTO') && queryStr.includes('RefreshTokens (UserId')) {
              ...
              if (queryStr && queryStr.includes('DELETE FROM') && queryStr.includes('RefreshTokens WHERE Token')) {
                  mockRefreshTokens = mockRefreshTokens.filter(rt => rt.Token !== this.parameters.Token);
                  saveMockRefreshTokens();
                  return { recordset: [] };
              }
              if (queryStr && (queryStr.includes('FROM dbo.RefreshTokens rt') || queryStr.includes('FROM RefreshTokens rt'))) {
                  ...
                  if (rt) {
                      ...
                      if (user) {
                          return { recordset: [{ ...rt, FullName: user.FullName, Email: user.Email, Role: user.Role || 'User', IsBanned: Boolean(user.IsBanned) }] };
                      }
                  }
                  return { recordset: [] };
              }
              ...
              if (queryStr && queryStr.includes('SELECT Id FROM') && queryStr.includes('Users WHERE Email')) {
                 const email = Object.values(this.parameters)[0];
                 const filtered = mockUsers.filter(u => u.Email === email);
                 return { recordset: filtered };
              }
...
module.exports = { getPool, sql, getMockState };
